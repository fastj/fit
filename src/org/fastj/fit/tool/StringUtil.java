...
public final class StringUtil {
	...
	{
		return splits(vstr, true);
	}
	
	public static final String[] splits(String vstr, boolean duplicate) throws DataInvalidException
	{
	...
	public static String expend(String content, ParameterTable table) throws ParamIncertitudeException, DataInvalidException
	{
		...
		while((idx = content.indexOf("${", offset)) >= 0)
		{
			...
			int sidx = readExpendBody(content, idx + 2, body);
			if (sidx > 0)
			{
				offset = sidx;
	...
	{
		return readCFParamStr(line, start, rlt, ',');
	}
	 ...
	 */
	public static int readCFParamStr(String line, int start, List<String> rlt, char spc)
	{
		//$[A-Za-z_0-9](.*)
		int len = line.length();
		int bigbCnt = 0;
		...
		for (; i < len; i++)
		{
			...
			if (status == 0 && bigbCnt == 0 && midbCnt == 0 && litbCnt == 0 && c == spc)
			{
				if (spc == ' '){
					String pv = trim(buff.toString());
					if (!pv.isEmpty()){
						rlt.add(pv);
					}
				}
	...
	{
		return readFuncParam(fparam, ',');
	}
	
	public static String[] readFuncParam(String fparam, char spc) throws DataInvalidException
	{
		if (fparam == null || fparam.isEmpty()) return new String[]{};
		List<String> l = new ArrayList<String>();
		int len = readCFParamStr(fparam + ")", 0, l, spc);
